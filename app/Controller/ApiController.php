...
App::uses('AppController', 'Controller');
	...
	public $uses = array('Attribute','AttributeProduct','AttributeQuestion','Brand','Category', 'CategoryQuestion','CategoryProduct','Product','Question','MaxWeightage');
		...
		public function api() {	
			...
			// BRANDS DATA		
			$params_brand		= array('fields'=>array('brand_id','brand_name','image_url','status','updated_date'), 'recursive'=>-1);
			...
			// PRODUCTS DATA
			$params_prd	= array('fields'=>array('product_id','product_name','product_image_url','brand_id','chicken','puppy','status','updated_date'), 
								'recursive'=>-1);
			...
			$params_cat_prod = array('fields'=>array('category_product_id', 'category_id', 'product_id','status','updated_date'), 'recursive'=>-1);
			$params_cat_prod += $conditions;			
			$this->PrepareData( $this->CategoryProduct->find('all', $params_cat_prod), $data_cat_prod, 0 );			
			$this->dataStatus($data_cat_prod, $status );
			if ( $status == 1)
				$final_data['Result']['Category-Product'] = $data_cat_prod;
