	...
	public function edit($id = null) {
		...
		if ( $this->request->is('post') || $this->request->is('put') ) {
			...
			if ( $this->Question->save( $data_question ) ) {		
				...
				// marked entire mapped data as deleted
				$cond2 =  'update attributes_questions set status = 0, updated_date = now() where question_id = ' . $id;
				$res = $this->AttributeQuestion->query( $cond2 );
				
				// now adding new data as updated
				$ctr = 0;
				foreach ( $DataProductAttributeIds as $key=>$attribute_id ){
					$data = array();
				...
				// marked entire mapped data as deleted			
				$cond =  'update categories_questions set status = 0, updated_date = now() where question_id='. $id ;
			...
			$temp_atts = $this->AttributeQuestion->find('all', array('fields'=>array('*'), 'conditions'=>array('AttributeQuestion.question_id'=> $id,'AttributeQuestion.status <> ' =>0 ) ) );			
			...
			$temp_cats = $this->CategoryQuestion->find('all', array('fields'=>array('*'), 'conditions'=>array('CategoryQuestion.question_id'=> $id, 'CategoryQuestion.status <> 0' ) ) );	
