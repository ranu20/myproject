...
class QuestionsController extends AppController {
	...
	public function add() {
		if ($this->request->is('post')) {
			...
			$data['Question']['question_type_id']	= $this->request->data['Question']['question_type_id'];			
		...
		$cond 			= array('fields'=>array('question_type_id','question_type'));
	...
	public function edit($id = null) {
		...
		if ( $this->request->is('post') || $this->request->is('put') ) {
			...
			$data_question['Question']['question_type_id']		= $this->request->data['Question']['question_type_id'];

			if ( $this->Question->save( $data_question ) ) {		

				// attribute update
				$cond2 =  'delete from attributes_questions where question_id = '. $this->request->data['Question']['question_id'];
				$res = $this->AttributeQuestion->query( $cond2 );
				
				$ctr = 0;
				foreach ( $DataProductAttributeIds as $key=>$attribute_id ){
					$data['question_id'] 	= $this->request->data['Question']['question_id'];
					$data['attribute_id'] 	= $attribute_id;
					$data['updated_date'] 	= date('Y-m-d H:i:s');
					$this->AttributeQuestion->create();
					$res = $this->AttributeQuestion->save( $data );
			...
			$temp_atts = $this->AttributeQuestion->find('all', array('fields'=>array('AttributeQuestion.attribute_id'), 'conditions'=>array('AttributeQuestion.question_id'=> $id ) ) );			
			...
			$temp_cats = $this->CategoryQuestion->find('all', array('fields'=>array('CategoryQuestion.category_id'), 'conditions'=>array('CategoryQuestion.question_id'=> $id ) ) );
